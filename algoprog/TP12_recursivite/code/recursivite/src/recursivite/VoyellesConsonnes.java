...
class VoyellesConsonnes {
    ...
    static int nbVoyellesAPartirDe(String mot, int pos) {
        int nbV = 0;
        if (mot.length() != 0) {
            if (estVoyelle(mot.charAt(pos))) {
                nbV += 1;
            }
        }
        if (pos < mot.length() - 1) {
            return nbV + nbVoyellesAPartirDe(mot, pos + 1);
        } else {
            return nbV;
        }
    }
    ...
    static int nbVoyellesRec(String mot) {
        return nbVoyellesAPartirDe(mot, 0);
    }
    ...
    static String voyellesPuisConsonnes(String mot) {
        char[] posVoyellesConsonnes = new char[mot.length()];
        int j = 0;
        for (int i = 0; i < mot.length(); i++) {
            if (estVoyelle(mot.charAt(i))) {
                posVoyellesConsonnes[j] = mot.charAt(i);
                j++;
            }
        }
        
        for (int i = mot.length() -1; i > -1; i--) {
            if (!estVoyelle(mot.charAt(i))) {
                posVoyellesConsonnes[j] = mot.charAt(i);
                j++;
            }
        ...
        String motRes = String.valueOf(posVoyellesConsonnes);
        return motRes;
    }
